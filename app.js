...
- "fill"タイプは穴埋め。"question"内の空欄は半角アンダースコア3つ「___」で表すこと。"blanks"は空欄の正解を出現順に並べた配列で、文中の「___」の数と必ず同じ長さにすること。【重要】空欄「___」は数式の「$...$」の【外】に置くこと（$...$ の中に ___ を入れてはならない）。数式中に空欄を作りたい場合は、空欄の前で数式を一度閉じ、後で開き直すこと。例：誤「$\\cos(___ + \\theta)$」／正「$\\cos($ ___ $+ \\theta)$」。また「$...$」には数式だけを入れ、日本語の文（「となり」「である」等）を混ぜないこと。"accept"（任意だが強く推奨）は各空欄の別解を入れた配列の配列。正解と同じ意味になる言い換え・同義語・別表記（例：「スカラー」⇔「スカラー値」、「二酸化炭素」⇔「CO2」、英語表記、送り仮名違い）を列挙すること。ただし【各空欄につき最大5個まで】とし、簡潔な語句に限ること。数式については、空白・記号の種類・積や和の順序の違いは採点側が自動で吸収するため不要だが、同じ量を別の標準的な書き方でも表せる場合（例：ノルム「‖a‖」と「√(x₁²+y₁²)」、内積「a・b」と「x₁x₂+y₁y₂」、「a・b=0」と「x₁x₂+y₁y₂=0」など）は、それらの別形も accept に含めること。
